...
const CreateLoginId = () => {
  ...
  // Suggested IDs
  const [suggestedIds, setSuggestedIds] = useState(() => generateSuggestionsForValue(initialId, idPrefix), );
                ...
                <Box className="mb-10">
                  ...
                  <Box className={` flex items-center w-[350px] h-[40px] rounded-lg border overflow-hidden  bg-white ${styles.border}`} >
                    ...
                    <Box className="w-[52px] flex justify-center">
                      {currentStatus?.icon && (
                        <Icon icon={currentStatus.icon} width={currentStatus.size} className={currentStatus.iconColor} />
                      )}
                    </Box>
                  </Box>
